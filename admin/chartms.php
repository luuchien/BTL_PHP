...
<script>
    var chartData; // Biến lưu trữ dữ liệu của biểu đồ

// Hàm cập nhật biểu đồ
function updateChart(data) {
    if(data != null){
        chart.setData(data);
        chartData = data; // Lưu trữ dữ liệu hiện tại của biểu đồ
    }
    else{
        alert("Không có dữ liệu nào được tìm thấy trong khoảng thời gian này!");
        // Xóa dữ liệu trước đó trên biểu đồ nếu không có dữ liệu mới được trả về
        chart.setData([]);
    }
}
...
$(document).ready(function() {
    filter30Day(); // Cập nhật biểu đồ khi trang được tải
});

// Hàm lọc dữ liệu trong 30 ngày gần nhất
function filter30Day(){
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            alert("Lỗi khi lấy dữ liệu từ máy chủ: " + textStatus);
            console.error("AJAX error:", errorThrown);
            // Xóa dữ liệu trước đó trên biểu đồ nếu có lỗi xảy ra
            chart.setData([]);
        }
    });
}
